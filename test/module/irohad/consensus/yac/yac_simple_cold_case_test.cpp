...
TEST_F(YacTest, YacWhenVoting) {
  ...
  YacHash my_hash(default_first_round_, "my_proposal_hash", "my_block_hash");
...
TEST_F(YacTest, YacWhenColdStartAndAchieveOneVote) {
  ...
  YacHash received_hash(default_first_round_, "my_proposal", "my_block");
...
TEST_F(YacTest, YacWhenColdStartAndAchieveCommitMessage) {
  YacHash propagated_hash(default_first_round_, "my_proposal", "my_block");
...
TEST_F(YacTest, PropagateCommitBeforeNotifyingSubscribersApplyReject) {
  ...
  RejectMessage reject(
      {vote,
       create_vote(YacHash(default_first_round_, "", "my_block"),
                   std::to_string(2 * f + 2))});
  commit.push_back(vote);
